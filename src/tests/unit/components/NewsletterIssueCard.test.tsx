...
const mockIssue = {
  ...
  summary: 'This week we cover the latest in AI, blockchain, and startup funding rounds.',
  issueNumber: 5,
...
describe('NewsletterIssueCard', () => {
  ...
  it('creates correct link to newsletter issue page', () => {
    ...
    expect(link).toHaveAttribute('href', '/newsletter/weekly-tech-roundup-issue-5');
  });
  ...
  it('displays calendar and user icons', () => {
    ...
    const calendarIcon = screen.getByText('Jan 15, 2024').previousElementSibling;
  ...
  it('truncates long titles with line-clamp', () => {
    const longTitleIssue = {
      ...mockIssue,
      title: 'This is a very long newsletter title that should be truncated after two lines to maintain consistent card layout',
    };
  ...
  it('truncates long summaries with line-clamp', () => {
    const longSummaryIssue = {
      ...mockIssue,
      summary: 'This is a very long summary that should be truncated after three lines to prevent the card from becoming too tall and maintain consistent layout across the newsletter archive grid.',
    };
  ...
  it('applies correct styling classes', () => {
    ...
    expect(title).toHaveClass('font-semibold', 'text-lg', 'group-hover:text-primary');
  });
