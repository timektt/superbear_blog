...
test.describe('Podcast and Newsletter Performance', () => {
  ...
  test('podcast listing page loads within performance budget', async ({ page }) => {
  ...
  test('newsletter archive page loads within performance budget', async ({ page }) => {
    const startTime = Date.now();
  ...
  test('newsletter content with rich text renders efficiently', async ({ page }) => {
    await page.route('**/api/newsletter/issues/newsletter-issue-1', async (route) => {
      const mockIssue = {
        issue: {
          id: '1',
  ...
  test('CSS animations are smooth', async ({ page }) => {
    ...
    const hasTransition = await podcastCard.evaluate(el => {
      ...
      return styles.transition.includes('200ms') || styles.transition.includes('0.2s');
    });
