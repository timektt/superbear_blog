...
test.describe('Public Site', () => {
  ...
  test('should display homepage with articles', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'SuperBear Blog' })).toBeVisible()
  ...
  test('should navigate to news page', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Latest Tech News' })).toBeVisible()
  })

  test('should display article cards with correct information', async ({ page }) => {
  ...
  test('should navigate to individual article page', async ({ page }) => {
    ...
    await page.getByRole('link', { name: /Test Article 1/ }).first().click()
  ...
  test('should handle loading states', async ({ page }) => {
    ...
    await page.route('**/api/articles', async (route) => {
      ...
      await route.fulfill({ json: { articles: [], total: 0, page: 1, limit: 10 } })
    })
