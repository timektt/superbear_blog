...
test.describe('Admin Interface', () => {
  ...
  test('should edit existing article', async ({ page }) => {
    ...
    await expect(page.getByLabel('Summary')).toHaveValue('This is a draft article')
  ...
  test('should delete article with confirmation', async ({ page }) => {
    ...
    await expect(page.getByText('Are you sure you want to delete this article?')).toBeVisible()
  ...
  test('should use rich text editor', async ({ page }) => {
    ...
    // Type in editor
    await page.locator('.ProseMirror').fill('This is test content for the article.')
