...
export default async function Home() {
  // Always use mock data in safe mode to prevent DB/Prisma issues
  if (!IS_DB_CONFIGURED) {
    return (
      <HomeView
        featured={{
  ...
  // In production with DB, import Prisma dynamically to avoid Edge runtime issues
  try {
    const { getSafePrisma } = await import('@/lib/prisma-safe');
        ...
        latest={
          latest.length > 0
            ? latest.map((article: any) => ({
                ...
                tags: article.tags?.map((tag: unknown) => tag.name) || [],
              }))
            : MOCK_LATEST
        }
      />
