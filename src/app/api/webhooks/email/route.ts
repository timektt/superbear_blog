      ...
      try {
        ...
        if (event.campaignId) {
          await prisma.campaignDelivery.updateMany({
            ...
            data: {
              ...
              ...(event.type === 'COMPLAINED' && { complainedAt: event.timestamp }),
            },
          });
        }
        ...
        if (['BOUNCED', 'COMPLAINED', 'UNSUBSCRIBED'].includes(event.type)) {
          await handleWebhookEvent({
            ...
            bounceType: event.bounceType === 'permanent' || event.bounceType === 'hard' ? 'hard' : 'soft',
            reason: event.reason,
            campaignId: event.campaignId,
          });
        }
        ...
        logger.error('Failed to process webhook event', error as Error, { event });
