...
import { createCampaign, getCampaigns, CampaignData } from '@/lib/email-campaigns';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data: CampaignData = {
      ...
      scheduledAt: validationResult.data.scheduledAt ? new Date(validationResult.data.scheduledAt) : undefined,
      recipientFilter: validationResult.data.recipientFilter ? {
        status: validationResult.data.recipientFilter.status,
        subscribedAfter: validationResult.data.recipientFilter.subscribedAfter ? new Date(validationResult.data.recipientFilter.subscribedAfter) : undefined,
        subscribedBefore: validationResult.data.recipientFilter.subscribedBefore ? new Date(validationResult.data.recipientFilter.subscribedBefore) : undefined,
      } : undefined,
    };
