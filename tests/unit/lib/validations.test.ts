...
describe('Validation Schemas', () => {
  describe('articleSchema', () => {
    ...
    it('should validate title length', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Title must be less than 200 characters')
      }
    })
    ...
    it('should validate slug format', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Slug must be URL-friendly')
      }
    })
    ...
    it('should validate summary length', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Summary must be less than 500 characters')
      }
    })
    ...
    it('should validate content structure', () => {
      ...
      expect(result.success).toBe(false)
      if (!result.success) {
        expect(result.error.issues[0].message).toContain('Content must be valid JSON')
      }
    })
