...
async function testCampaignSystem() {
  ...
  try {
    ...
    console.log(
      `   👥 Target: ${testCampaignData.recipientFilter.status.join(', ')} subscribers\n`
    );
    ...
    if (recentArticles.length > 0) {
      console.log(
        `   ✅ Found ${recentArticles.length} recent articles for content generation:`
      );
      recentArticles.forEach((article, index) => {
        console.log(
          `      ${index + 1}. ${article.title} (${article.category.name})`
        );
      });
    } else {
      console.log(
        '   ⚠️  No published articles found. Create some articles first.'
      );
    }
    console.log('');
    ...
    console.log(
      `   🔐 Cron Secret: ${process.env.CRON_SECRET ? 'Set' : 'Not set'}`
    );
    console.log('');
    ...
    console.log(
      `   ⏰ Found ${scheduledCampaigns.length} campaigns ready to be sent`
    );
    ...
    if (scheduledCampaigns.length > 0) {
      ...
      scheduledCampaigns.forEach((campaign) => {
        console.log(
          `      - ${campaign.title} (scheduled for ${campaign.scheduledAt})`
        );
      });
    }
    console.log('');
    ...
    console.log(
      `✅ Newsletter subscribers: ${subscriberCount} total subscribers`
    );
    console.log(
      `✅ Content generation: ${recentArticles.length} articles available`
    );
    console.log(
      `✅ Environment config: ${missingEnvVars.length === 0 ? 'Complete' : 'Incomplete'}`
    );
    console.log(
      `✅ Scheduled campaigns: ${scheduledCampaigns.length} ready to send`
    );
    console.log('');
    ...
    if (
      ...
      console.log(
        '2. Set up a cron job to call /api/cron/campaigns every 5 minutes'
      );
      console.log(
        '3. Configure your production email service (SendGrid, Mailgun, etc.)'
      );
    } else {
